...
- **匹配策略**:
  ...
  - **通配符匹配** *(v1.2 新增)*:
    - `*` 匹配任意数量的字符
    - `?` 匹配单个字符
    - 示例：`熬了*通宵` 匹配 "熬了3个通宵"、"熬了一晚上通宵" 等
    - 自动识别：输入包含 `*` 或 `?` 的关键词将自动启用通配符匹配
  - **关键词作用域** *(v1.3 新增)*:
    - **全局关键词** (默认): 作用于所有启用过滤的网站。
    - **站点独立关键词**: 仅在指定的网站上生效。例如 `！` 仅作用于 `bilibili.com`。
    - 用户在添加关键词时可选择作用域（全局 或 特定站点）。
    - 向后兼容：未指定作用域的关键词（包括旧版导入数据）自动视为全局关键词。
...
- **导出功能**: 用户可将当前所有关键词导出为 JSON 文件备份。
  - 导出文件包含版本号、导出时间戳和完整关键词列表（含过期时间和作用域）。
...
- v1.3 新增可选 `scope` 字段 `{word, expiresAt, scope?}`，旧数据无需迁移（缺失 scope = 全局）。
- 迁移过程透明，无需用户手动操作。
...
2. **Popup 弹窗**:
   ...
   - 开关: [开启/关闭] (针对当前站点)。
   - 快速添加框: [ 输入关键词 ] [作用域选择器: 🌐 全局 / 📌 仅当前站点] [添加]。
   - 链接: [管理所有关键词] (跳转至选项页)。
3. **设置页 (Options)**:
   - 关键词列表管理 (Tag 形式，支持删除，显示过期时间和作用域徽章)。
   - 添加关键词区域 (含作用域选择器、有效期选择器：永久/30天)。
